...
class ReportController extends \yii\web\Controller {
    ...
    public function actionReport1() {
        $sql = 'select count(m.id) as mid, concat(p.firstname," ",p.lastname) as fullname  
                from mission m
        ...
        foreach ($rawData as $d) {
            $graph[] = ['type' => 'column', 'name' => $d['fullname'], 'data' => [intval($d['mid'])]];
        }
        $dataProvider = new ArrayDataProvider([
            'allModels' => $rawData,
            'sort' => [
                'attributes' => ['mid', 'fullname']
            ]
        ]);
...
";
    ...
    public function actionReport3() {
        ...
        $pdf = new Pdf([
            ...
            'methods' => [ 
                'SetHeader'=>['ทดลองการใช้ PDF ด้วย mpdf'], 
                'SetFooter'=>['{PAGENO}'],
            ]
        ]);
